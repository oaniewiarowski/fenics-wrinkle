...
class InputOutputHandling:
    def __init__(self, membrane):
        """
        This class handles reading and writing the simulation state
        """
...
class XDMFFileIO:
    ...
    def _write_xdmf(self):
        ...
        if self.xdmf_first_output:
            ...
            self.V = dolfin.VectorFunctionSpace(mem.mesh, 'CG', 1, dim=mem.nsd) # in case of periodic V, visualization is wrong
            self.delta_gammah =  dolfin.interpolate(mem.gamma, self.V)

            # Get the displacement from the parametric to the reference for plotting

            self.gamma_paraview = dolfin.Function(self.V)
            self.gamma_paraview.rename('gamma','Parametric mapping')
            self.gamma_paraview.vector()[:] = self.delta_gammah.vector()[:] - dolfin.interpolate(self.idn, self.V).vector()[:]
...
#            self.W = dolfin.TensorFunctionSpace(mem.mesh, "CG", 2, shape=(3, 3))
        ...
        self.uh.assign(dolfin.project(self.gamma_paraview + mem.data['u'], self.V))
        # self.uh.vector()[:] = self.gamma_paraview.vector()[:] + mem.q.split(deepcopy=True)[0].vector()[:]
        self.xdmf_file.write(self.uh, t)
        self.xdmf_file.write(self.gamma_paraview, t)
