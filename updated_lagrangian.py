...
from fenics_wrinkle.geometry.cylinder import Cylinder
...
input_dict = {
        ...
        'Inflation Solver': 'Custom Newton'}
...
if True: #self.input_dict['Boundary Conditions'] == 'Roller':
    front_bd = df.CompiledSubDomain("(near(x[1], 0) && on_boundary)")
    back_bd = df.CompiledSubDomain("(near(x[1], 1) && on_boundary)")
    mesh_func = df.MeshFunction("size_t", membrane.mesh, membrane.mesh.topology().dim()-1)
    front_bd.mark(mesh_func, 1)
    back_bd.mark(mesh_func, 2)
    df.File("mesh_func.pvd") << mesh_func
    membrane.ds = df.Measure('ds', domain=membrane.mesh, subdomain_data=mesh_func)
...
if True:# input_dict['Boundary Conditions'] == 'Roller':
    front_bd = df.CompiledSubDomain("(near(x[1], 0) && on_boundary)")
    back_bd = df.CompiledSubDomain("(near(x[1], 1) && on_boundary)")
    mesh_func = df.MeshFunction("size_t", mem.mesh, mem.mesh.topology().dim()-1)
    front_bd.mark(mesh_func, 1)
    back_bd.mark(mesh_func, 2)
...
for i in range(10):
    ...
    _ = prob.add_var(mem.V, bc=mem.bc)
    prob.var[0] = u
    ...
    NORM_u.append(df.errornorm(u, u_old))
...
def plot_result():
    ...
    ax[2].plot(NORM_u, '-*', label = r'$|u-\overline{u}|$')
    ax[2].legend()
